...
function writeHtml(v) {
    ...
    function getAppTmp() {
        ...
        //於bk前插入, 添加組件kbname
        $('div.bk').before(`<div class="head1"><span style="cursor:pointer;" title="open for copy link to view component" onclick="window.open('//yuda-lyu.github.io/w-geo-vue/examples/app.html?cmp='+this.innerText,'_blank')">${v.kbname}</span></div>`)
    ...
    let opt = {
        ...
        <!-- w-geo-vue -->
        <script src="../dist/w-geo-vue.umd.js"></script>
        ...
        appStyle: ``,
        appTmp: getAppTmp(),
        installVue: `Vue.use(window['w-geo-vue'])`,
        data: v.data,
        mounted: v.mounted,
        computed: v.computed,
        methods: v.methods,
